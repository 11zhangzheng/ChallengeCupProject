<template>
  <div class="mall-container">
    <!--回到顶部-->
    <el-backtop style="height: 50px;width: 50px" target=".mall-container">
    ...
    <div class="mall-nav">
      <el-menu :collapse-transition="false" :default-active="activeIndex" active-text-color="#409EFF"
        ...
        <el-menu-item index="1" style="width: 12%;margin-left:10%;position: relative">
          <router-link style="text-decoration: none;position: absolute;right: 0" to="/MallHome">
        ...
        <el-menu-item index="2" style="width: 50%">
          <el-input v-model="queryProductInfo" class="input-with-select"
            ...
            <el-select slot="prepend" v-model="select" placeholder="请选择" style="width: 120px;user-select: none;">
    ...
    <!--轮播图-->
    <div v-if="this.bannerInfo" class="slideshow">
    ...
    <div class="new-product">
      <div class="box-hd">
        <h2 class="new-product-title" style="width: 98.5%;">
          ...
          <div style="position: absolute;top: 6px;right: 16px;">
            <el-button :disabled="butLFlag" icon="el-icon-arrow-left" size="mini" style="padding: 5px 12px;margin: 0"
                       @click="newLeft()"></el-button>
            <el-button :disabled="butRFlag" icon="el-icon-arrow-right" size="mini" style="padding: 5px 12px;margin: 0"
                       @click="newRight()"></el-button>
          </div>
        </h2>
      </div>
      <div style="overflow: hidden">
        <div class="new-sale">
          ...
          <el-image class="animate__animated animate__flip" src="https://spring-oss.oss-cn-beijing.aliyuncs.com/images/img/nurse.png"
                    style="width: 100px;padding: 20px;"></el-image>
          <h3>新品专区</h3>
        </div>
        <div ref="newInfo" class="new-product-info">
          <ul ref="newUl" class="sale-list">
            <li v-for="(product,index) in this.newProduct" :key="index" ref="newLi" @click="buy(product.productId)">
              <el-image :src="product.productUrl" class="img"></el-image>
              <h3 class="title">
                <el-tag v-if="product.isNew" size="mini" style="margin-right: 3px" type="warning">新品</el-tag>
                {{ product.productName }}
              </h3>
    ...
    <div class="product clearfix">
      <div v-for="(value, key) in this.productsInfo" :key="key" class="product-info">
        <el-image :src="value[0]['urlTop']" lazy
                  scroll-container=".mall-container"
                  style="height: 100px;margin: 30px 0 0;width: 98.5%;min-width:1325px"></el-image>
        <div class="box-hd">
        ...
        <ul class="product-type-promo">
          <li>
            <el-image :src="value[0]['urlLeft']" lazy scroll-container=".mall-container"
                      style="height: 100%;width: 100%"
                      @click.native="queryProduct(value[0]['typename'],'type')"></el-image>
          </li>
        </ul>
        <ul v-if="value[1].length!==0" class="product-item">
          <li v-for="(product,index) in value[1]" :key="index" @click="buy(product.productId)">
            <a>
              <el-image :src="product.productUrl" lazy scroll-container=".mall-container"></el-image>
              <h3 class="title">
                <el-tag v-if="product.isNew" size="mini" style="margin-right: 3px" type="warning">新品</el-tag>
                {{ product.productName }}
              </h3>
        ...
        <div v-else class="clearfix"
             style="float:left;display:flex;justify-content:center;align-items: center; width:83%;height: 75%;background-color: #FFF">
          <el-image src="https://spring-oss.oss-cn-beijing.aliyuncs.com/images/img/noproduct.png"
                    style="width: 410px"></el-image>
        </div>
      </div>
    </div>
    <!--底部-->
