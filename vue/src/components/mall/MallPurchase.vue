<template>
  <div class="mall-purchase">
    <div class="product-box">
      <div class="nav-bar">
        <div v-if="productInfo" class="container">
          <el-image :src="productInfo.productUrl"
                    class="logo animate__animated animate__rubberBand"
                    style="width: 65px;height: 65px;margin-top: 8px;margin-right: 10px;"></el-image>
          ...
          <el-link type="danger" @click="toReview">农产品评价</el-link>
        </div>
      </div>
    </div>
      ...
      <div class="product-info-content container">
        ...
        <div class="product-con">
          ...
          <div class="price-info">
            ...
            <span v-else>农产品库存 {{ productInfo.productStock }}</span>
          </div>
          <div class="line"></div>
          <div class="address-box container">
            <i class="el-icon-location-outline"></i>
            <div class="address-con">
              <div v-if="order.acceptAddress" class="address-info">{{ order.acceptAddress }}</div>
              <div v-else class="address-info">未设置</div>
            </div>
            <div class="address-alter" @click="addressFormVisible=true">修改</div>
          </div>
          <div v-if="this.productSpec!==null && this.productSpec.length!==0" class="option-box">
            <div class="title">选择规格</div>
            <el-radio-group v-for="(option,index) in this.productSpec" :key="index" v-model="order.productSpecs"
                            class="option-list" fill="#ff6700">
              <el-radio-button :label="option"></el-radio-button>
            </el-radio-group>
          </div>
          <div class="select-amount">
            <div style="font-size: 18px;">选择数量</div>
            <el-input-number v-model="order.payAmount" :max="productInfo.productStock" :min="1"
                             style="margin: 16px;" @change="calculationPrice"></el-input-number>
          </div>
          <div class="selected-list container">
            ...
            <div>
              <div v-model="totalPrice" class="total-price">总计：{{ totalPrice }}元</div>
              <el-button plain style="float: left;margin: 20px 40px" type="success" @click="joinCart">加入购物车
              </el-button>
              <el-button plain style="float: left;margin: 20px 0" type="danger" @click="buy">立即购买</el-button>
            </div>
          </div>
        </div>
      </div>
    ...
    <el-dialog :visible.sync="orderFormVisible" title="订单信息" width="600px">
      <el-form ref="order" :model="order" :rules="orderRules" class="order-form">
        <el-form-item label="农产品名称">
          <el-input v-model="this.productInfo['productName']" disabled></el-input>
        </el-form-item>
        <el-form-item v-if="order.productSpecs" label="农产品规格">
        ...
        <el-form-item v-if="isVip" label="会员折扣" style="position: relative">
          ...
          <el-button class="vip-info" plain>VIP {{ discountStrength }}折</el-button>
        </el-form-item>
    ...
    if (this.buyId != null) {
      ...
      this.$http.post('/productSpecs/findAllByProId?productId=' + this.buyId).then((rep) => {
        if (rep.data.code === 200) {
          ...
          this.order.productSpecs = this.productSpec[0]; //默认选择第一个农产品规格
        } else {
          this.$msg.error(rep.data.message);
        }
      }).catch((err) => {
