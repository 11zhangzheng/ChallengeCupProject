<template>
  <div class="shopping-cart">
    <el-button v-if="this.tableData && tableData.length!==0" circle class="clear-cart"
               icon="el-icon-delete-solid" title="清空购物车" type="primary" @click="clearCart"></el-button>
    <div class="product-box">
      <div class="nav-bar">
        <div class="container">
          <router-link style="text-decoration: none" title="官网" to="/MallHome">
    ...
    <div v-if="tableData" class="page-main">
      <div v-if="tableData.length!==0" class="cart-info">
        <el-table ref="cartTable" :data="tableData" :highlight-current-row="false" style="width: 100%;"
            ...
            <template slot-scope="scope">
              <router-link :to="'/MallPurchase?id='+scope.row.productId" style="text-decoration:none;">
                <div class="product-name">
                  <el-image :src="scope.row.productUrl" class="product-img"></el-image>
                  <span class="name">{{ scope.row.productName }}</span>
                </div>
              </router-link>
            </template>
          </el-table-column>
          <el-table-column label="规格" prop="productSpecs" width="150"></el-table-column>
          ...
          <el-table-column label="数量" width="180">
            <template slot-scope="scope">
              <el-input-number v-model="scope.row.payAmount" :max="scope.row.productStock" :min="1"
                               size="small" @change="calculationPrice(scope.$index)"></el-input-number>
            </template>
          </el-table-column>
          <el-table-column label="会员折扣" width="120">
            <template slot-scope="scope">
              <span v-if="scope.row.discount===100">无</span>
              <el-tag v-else type="warning">{{ scope.row.discount }}折</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="150">
            <template slot-scope="scope">
              <el-button circle icon="el-icon-delete" type="danger" @click="deleteProduct(scope.$index)"></el-button>
            </template>
        ...
        <div class="cart-bar">
          ...
          <div class="section-right">
            ...
            <el-tooltip v-if="selectedData" :disabled="selectedData.length!==0" content="请勾选需要结算的农产品"
                        effect="light" placement="top">
              <div class="button">
                <el-button :disabled="selectedData.length===0" style="height: 100%;width: 100%" type="danger"
                           @click="buy">去结算
                </el-button>
              </div>
            </el-tooltip>
    ...
    <el-dialog :visible.sync="addressFormVisible" class="address-dialog" title="收货地址" width="450px">
    ...
    <!--产生订单-->
    <el-dialog :visible.sync="orderFormVisible" title="订单信息" width="600px">
      <el-form ref="order" :model="baseInfo" :rules="baseInfoRules" class="order-form">
        ...
        <el-form-item label="收货地址" prop="acceptAddress" style="position: relative">
          ...
          <el-button plain style="position: absolute;top:1px;right: 14px;border: none" @click="addressFormVisible=true">
            修改
          </el-button>
        </el-form-item>
    ...
    createOrder() {
      ...
      this.$http.post('/order/cartOrder', this.$qs.stringify(send)).then((rep) => {
        ...
        if (rep.data.code === 200) {
          ...
          let msg = '共' + productNo.length + '农产品购买成功，需支付 ' + money + ' 元';
          this.$alert(msg, '下单成功', {
...
@keyframes myAnimation {
  0% {
    transform: rotateY(0deg) scale(.9, .9);
  }
  50% {
    transform: rotateY(180deg) scale(.9, .9);
  }
  100% {
    transform: rotateY(360deg) scale(.9, .9);
  }
}
