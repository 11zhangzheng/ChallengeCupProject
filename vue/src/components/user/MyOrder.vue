<template>
  <div class="my-order">
    <!--导航栏-->
    <div class="mall-nav">
      <el-menu :collapse-transition="false" :default-active="activeIndex" active-text-color="#409EFF"
               class="el-menu-demo" mode="horizontal" @select="handleSelect">
        <el-menu-item index="1" style="width: 22%">
          <router-link style="text-decoration: none" to="/MallHome">

          </router-link>
        </el-menu-item>
        <el-menu-item index="2" style="width: 50%">
          <el-input v-model="queryProductInfo" class="input-with-select"
                    placeholder="请输入内容" style="height: 45px;"
        ...
        <el-submenu index="3" style="width: 10%">
          ...
          <el-menu-item v-for="(item,i) in productType" :key="'3-'+i" :index="'3-'+i"
                        @click="queryProduct(item,'type')">{{ item }}
          </el-menu-item>
        </el-submenu>
        <el-submenu index="4" style="width: 10%">
          <template slot="title">农产品品牌</template>
          <el-menu-item v-for="(item,i) in productBrand" :key="'4-'+i" :index="'4-'+i"
                        @click="queryProduct(item,'brand')">{{ item }}
          </el-menu-item>
        </el-submenu>
      </el-menu>
    </div>
    <div v-if="this.orderData.length!==0" class="order-box">
      <el-tabs v-model="activeName" class="order-list" stretch @tab-click="handleClick">
        <el-tab-pane label="全部订单" name="first">
          <el-table :data="orderData" center class="order-table">
            <el-table-column type="expand">
              <template slot-scope="props">
                <el-form class="product-form" inline label-position="left">
                  <el-form-item label="农产品名称">
                    <span>{{ props.row.productName }}</span>
                  </el-form-item>
                  <el-form-item v-if="props.row.productSpecs" label="农产品规格">
                    <span>{{ props.row.productSpecs }}</span>
                  </el-form-item>
                  <el-form-item label="农产品类别">
                    <span>{{ props.row.productType }}</span>
                  </el-form-item>
                  <el-form-item label="农产品单价">
            ...
            <el-table-column align="center" label="订单编号" prop="orderNo" width="180"></el-table-column>
            <el-table-column align="center" label="农产品编号" prop="productNo" width="180"></el-table-column>
            <el-table-column align="center" label="下单时间" prop="orderTime" width="200"></el-table-column>
            <el-table-column align="center" label="付款金额" prop="payPrice" width="150"></el-table-column>
            <el-table-column align="center" label="支付方式" prop="payType" width="120"></el-table-column>
            <el-table-column align="center" label="订单状态" prop="orderState" width="120"></el-table-column>
            <el-table-column align="center" label="操作">
              <template slot-scope="scope">
                <el-button
                  v-if="scope.row.orderState==='待付款'"
                  size="mini"
                  @click="payment(scope.row.orderNo,scope.row.orderTime,scope.row.productName,scope.row.payPrice,scope.row.productType)">
                  农产品付款
                </el-button>
                <el-button
                  v-if="scope.row.orderState==='已发货'"
                  size="mini"
                  @click="receiveGoods(scope.row.orderId)">确认收货
                </el-button>
                <el-button
                ...
                <el-button
                  ...
                  size="mini"
                  type="danger"
                  @click="cancelOrder(scope.row.orderId,scope.row.orderTime)">取消订单
                </el-button>
                <el-button
                ...
                <el-button
                  ...
                  size="mini"
                  type="primary"
                  @click="sendBack(scope.$index)">上门取件
                </el-button>
                <el-button
                ...
                <el-button
                  ...
                  size="mini"
                  type="primary"
                  @click="review(scope.row.orderNo,scope.row.productNo,scope.row.productName,scope.row.productSpecs)">评价
                </el-button>
                <el-button
                  ...
                  size="mini"
                  type="danger"
                  @click="applyReturn(scope.row.orderId,scope.row.orderNo,scope.row.payPrice,scope.row.orderState)">申请退货
                </el-button>
                <el-button
                  ...
                  size="mini"
                  type="primary"
                  @click="applyReturn(scope.row.orderId,scope.row.orderNo,scope.row.payPrice,scope.row.orderState)">申请退款
                </el-button>
                <el-button
                  v-if="scope.row.orderState==='退货完成' || scope.row.orderState==='已评价'"
                  size="mini"
                  type="info"
                  @click="delOrder(scope.row.orderId)">隐藏订单
                </el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <div v-else class="no-order">
      <el-image src="https://spring-oss.oss-cn-beijing.aliyuncs.com/images/img/noorder.png"
                style="width:400px;height:388px"></el-image>
    </div>
    <el-dialog :visible.sync="returnFormVisible" class="return" title="提示" top="30vh" width="416px">
      <el-form ref="returnFrom" :model="returnFrom" :rules="returnRules">
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogFormVisible" center class="logistics"
               title="物流信息" width="800px">
      <el-form ref='changeFrom' :model="alterForm" :rules="alterRules" class="info-form" inline>
        <el-form-item label="订单编号：" prop="orderNo">
          <el-input v-model="alterForm.orderNo" placeholder="订单编号" readonly size="small"></el-input>
        </el-form-item>
        <el-form-item label="快递公司：" prop="parcelName">
          <el-select v-model="alterForm.parcelName" filterable placeholder="快递公司" size="small">
            <el-option v-for="(item,index) in Parcel" :key="index" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="收货人：" prop="receiver">
          <el-input v-model="alterForm.receiver" placeholder="收货人" size="small"></el-input>
        </el-form-item>
        <el-form-item label="收货人联系方式：" prop="receiverTel">
          <el-input v-model="alterForm.receiverTel" placeholder="收货人联系方式" readonly size="small"></el-input>
        </el-form-item>
        <el-form-item label="收货人地址：" prop="contactWay" style="width: 100%">
          <el-input v-model="alterForm.receiverAdd" placeholder="收货人地址" readonly size="small"></el-input>
        </el-form-item>
        <el-form-item label="发货人：" prop="sender">
          <el-input v-model="alterForm.sender" placeholder="发货人" size="small"></el-input>
        </el-form-item>
        <el-form-item label="发货人联系方式：" prop="senderTel">
          <el-input v-model="alterForm.senderTel" placeholder="发货人联系方式" size="small"></el-input>
        </el-form-item>
        <el-form-item label="发货人地址：" prop="senderAdd">
          <el-input v-model="alterForm.senderAdd" placeholder="发货人地址" size="small"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button style="padding: 12px 30px;margin: 0 30px;" type="primary" @click="submitInfo('changeFrom')">确 定
        </el-button>
        <el-button style="padding: 12px 30px;margin: 0 30px;" @click="cancelChange('changeFrom')">取 消</el-button>
      </div>
    </el-dialog>
    <el-dialog :visible.sync="serviceVisible" style="padding: 0 20px 0" title="客服" width="30%">
    ...
    <el-dialog :visible.sync="reviewFormVisible" class="review" title="农产品评价" top="20vh" width="500px">
      <el-form ref="reviewFrom" :model="reviewFrom" :rules="reviewRules">
        <el-form-item label="农产品名称" prop="productName" required>
          <el-input v-model="reviewFrom.productName" placeholder="农产品名称" readonly size="small"></el-input>
        </el-form-item>
        <el-form-item v-if="reviewFrom.productSpecs" label="农产品规格" prop="productSpecs" required>
          <el-input v-model="reviewFrom.productSpecs" placeholder="农产品规格" readonly size="small"></el-input>
        </el-form-item>
        <el-form-item class="star-item" label="描述相符" prop="starLevel" required style="margin-bottom: 0">
          <el-rate v-model="reviewFrom.starLevel" :colors="colors" show-text></el-rate>
        </el-form-item>
        <el-form-item label="农产品评价" prop="productReview">
          <el-input v-model="reviewFrom.productReview" :rows="3" placeholder="从多个角度评价宝贝，可以帮助更多想买的人"
                    style="max-height: 120px" type="textarea"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      reviewRules: {
        ...
        productReview: [{required: true, message: '请填写农产品评价', trigger: 'blur'},
  ...
  methods: {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http.post('/productReview/add', this.$qs.stringify(this.productReview, {skipNulls: true})).then((rep) => {
            ...
            if (rep.data.code === 200) {
              this.reqInfo();
              this.$notify({title: '成功', message: '农产品评价提交成功', type: 'success'});
              this.reviewFormVisible = false;
              this.$refs[formName].resetFields();
            } else {
              this.$notify.error({title: '失败', message: "农产品评价失败"});
            }
          }).catch((err) => {
            loading.close();
            this.$msg.error(err)
    ...
    receiveGoods(orderId) {
      this.$confirm('请收到农产品并检查无误后，再确认收货', '提示', {
