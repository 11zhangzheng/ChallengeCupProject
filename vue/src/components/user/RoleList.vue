  ...
  <div class="role-list">
    <el-card class="user-hd" shadow="never">
      <div slot="header" style="text-align: left;">
        ...
        <span>筛选搜索</span>
        <el-button class="el-button-search" style="right: 111px" type="primary" @click="queryRoleInfo()">查询
        </el-button>
        <el-button class="el-button-search" style="right: 20px" @click="resetForm('roleInfoForm')">重置</el-button>
      </div>
      <el-form ref="roleInfoForm" :model="roleInfoForm" class="query-form" inline>
        <el-form-item label="输入搜索：" prop="key">
          <el-input v-model="roleInfoForm.key" clearable placeholder="角色名称/描述"
                    @keyup.enter.native="queryRoleInfo"></el-input>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="role-box" shadow="never">
      ...
      <el-table :data="roleInfo" border class="type-table" height="425" style="width: 100%">
        <el-table-column label="编号" prop="roleId" width="150"></el-table-column>
        <el-table-column label="角色" prop="roleName" width="300"></el-table-column>
        <el-table-column label="描述" prop="roleDescribe" width="500"></el-table-column>
        <el-table-column label="是否启用" width="150">
          <template slot-scope="scope">
            <el-switch v-model="roleInfo[scope.$index].roleState"
                       :disabled="roleInfo[scope.$index].roleName.indexOf(VAR.adminRole)!==-1||
                        roleInfo[scope.$index].roleName.indexOf(VAR.ordinaryRole)!==-1" @change="isRoleState(scope.$index)"/>
          </template>
        </el-table-column>
        <el-table-column align="center" label="操作" min-width="200">
          <template slot-scope="scope">
            <el-button :disabled="roleInfo[scope.$index].roleName.indexOf(VAR.adminRole)!==-1||
                        roleInfo[scope.$index].roleName.indexOf(VAR.ordinaryRole)!==-1" size="mini"
                       style="margin: 0 2%" @click="openChange(scope.$index)">编辑
            </el-button>
            <el-button slot="reference" :disabled="roleInfo[scope.$index].roleName.indexOf(VAR.adminRole)!==-1||
                        roleInfo[scope.$index].roleName.indexOf(VAR.ordinaryRole)!==-1"
                       size="mini" style="margin: 0 2%" type="danger" @click="deleteRole(scope.$index, roleInfo)">
              删除
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="roleDialogVisible" center class="user-dialog"
               title="编辑角色信息">
      <el-form ref='changeFrom' :model="alterForm" :rules="alterRules" class="info-form" inline>
        <el-form-item label="角色名称：" prop="roleName" style="width: 100%">
          <el-input v-model="alterForm.roleName" placeholder="角色名称" size="small"></el-input>
        </el-form-item>
        <el-form-item label="角色描述：" prop="roleDescribe" style="width: 100%">
          <el-input v-model="alterForm.roleDescribe" autosize placeholder="角色描述" size="small"
  ...
  methods: {
          ...
          if (this.insert) {
            this.$http.post('/role/existRoleName?roleId=0&roleName=' + this.alterForm.roleName).then((rep) => {
              if (rep.data.code === 200) {
                if (rep.data.data) {
                  this.$alert('角色(' + this.alterForm.roleName + ')\t已存在，不能重复添加', '警告', {
                  ...
                  this.$http.post('/role/add', this.$qs.stringify(this.alterForm)).then((rep) => {
                    ...
                    if (rep.data.code === 200) {
                      ...
                      this.$notify({
                        title: '成功',
                        message: '角色(' + this.alterForm.roleName + ')\t添加成功',
                        type: 'success'
                      });
                    } else {
                ...
                if (rep.data.data) {
                  ...
                  this.$http.post('/role/update', this.$qs.stringify(this.alterForm)).then((rep) => {
                    ...
                    if (rep.data.code === 200) {
                      ...
                      this.$notify({
                        title: '成功',
                        message: '角色(' + this.roleInfo[this.handleIndex].roleName + ')\t信息更新成功',
                        type: 'success'
                      });
    ...
    deleteRole(index, rows) {
      ...
      this.$http.post('/userRole/existsRole?roleId=' + this.roleInfo[index].roleId).then((rep) => {
        if (rep.data.code === 200) {
          if (rep.data.data) {
            this.$alert('角色(' + name + ')\t下存在用户，无法删除', '警告', {
              confirmButtonText: '确定',
              type: "warning"
            });
          } else {
