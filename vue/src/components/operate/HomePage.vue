...
<template>
  <div class="home-page">
    ...
    <div class="user-info">
      <!--头像图片-->
      <el-avatar :size="64" :src="this.user.avatarUrl" class="avatarImg" shape="square">
        <img
          alt="默认头像"
          src="https://th.bing.com/th?id=OIP.rlcgwt3Z_xYBJqNlE1lLiQAAAA&w=119&h=104&c=7&bgcl=d5f5d8&r=0&o=6&dpr=2.2&pid=13.1"/>
      </el-avatar>
      <!--用户信息-->
      <div class="info-box">
    ...
    <div class="order-info">
      ...
      <div class="order-total">
        ...
        <h3>今日订单总数</h3>
        <countTo :duration='5000' :endVal='dataInfo.todayOrderCount' :startVal='0' useEasing></countTo>
      </div>
      <!--今日销售总额-->
      <div class="sales-total">
        <i class="el-icon-bank-card"></i>
        <h3>今日销售总额</h3>
        <countTo :duration='5000' :endVal='dataInfo.todayOrderMoney' :startVal='0' prefix="￥"></countTo>
      </div>
      <!--昨日销售总额-->
      <div class="yesterday-sales-total">
        <i class="el-icon-coin"></i>
        <h3>昨日销售总额</h3>
        <countTo :duration='5000' :endVal='dataInfo.yesterdayOrderMoney' :startVal='0' prefix="￥" useEasing></countTo>
      </div>
    </div>
    <!--订单处理-->
    <div v-if="currentRole.indexOf(this.$VAR.adminRole)!==-1 || currentRole.indexOf(this.$VAR.orderRole)!==-1"
         class="order-deal">
      <div class="title">
        <h3>待处理订单</h3>
      </div>
    ...
    <!--农产品信息-->
    <div v-if="currentRole.indexOf(this.$VAR.adminRole)!==-1 || currentRole.indexOf(this.$VAR.productRole)!==-1"
         class="product-info">
      <div class="title">
        <h3>农产品总览</h3>
      </div>
    ...
    <div class="order-analyze">
      ...
      <div class="analyze-box">
        <div class="block">
          <el-date-picker v-model="dateTime" :default-value="new Date()" :picker-options="pickerOptions" end-placeholder="结束日期"
                          range-separator="至"
                          size="mini"
                          start-placeholder="开始日期" style="width: 280px" type="daterange"
                          unlink-panels value-format="yyyy-MM-dd" @change="dealData">
          </el-date-picker>
        </div>
        <div class="ring-chart">
          <div class="total-data">
            <span style="color: #FF7993;">订单总数
              <countTo :duration='3000' :endVal='phaseOrderCount' :startVal='0' useEasing></countTo>
            </span>
            <span style="color: #63C2FF; padding-left: 30px;">销售总额
              <countTo :duration='3000' :endVal='phaseOrderAmount' :startVal='0' prefix="￥" useEasing></countTo>
            </span>
          </div>
          <ve-ring :data="ringData" :settings="ringSettings" height="330px">
            <div v-if="ringEmpty" class="data-empty">暂无数据😂</div>
          </ve-ring>
        </div>
        <div class="line-chart">
          <ve-line :data="lineData" :extend="lineExtend" :settings="lineSettings" height="350px" width="100%">
