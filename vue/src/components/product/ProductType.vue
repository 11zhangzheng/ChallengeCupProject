  ...
  <div class="product-type">
    <el-card class="product-hd" shadow="never">
      <div slot="header" style="text-align: left;">
        ...
        <span>筛选搜索</span>
        <el-button class="el-button-search" style="right: 111px" type="primary" @click="queryProductType()">查询
        </el-button>
        <el-button class="el-button-search" style="right: 20px" @click="resetForm('productTypeForm')">重置</el-button>
      </div>
      <el-form ref="productTypeForm" :model="productTypeForm" class="query-form" inline>
        <el-form-item label="输入搜索：" prop="key">
          <el-input v-model="productTypeForm.key" clearable placeholder="类别名称/关键字"
                    @keyup.enter.native="queryProductType"></el-input>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="product-box" shadow="never">
      ...
      <el-table :data="productType" border class="type-table" height="425" style="width: 100%">
        <el-table-column label="编号" prop="typeId" width="100"></el-table-column>
        <el-table-column label="类别名称" prop="typeName" width="150"></el-table-column>
        <el-table-column label="类别描述" prop="typeDescribe" width="400"></el-table-column>
        <el-table-column align="center" label="横幅宣传图" width="300">
        ...
        <el-table-column align="center" label="左侧宣传图" width="300">
          <template slot-scope="scope">
        ...
        <el-table-column align="center" label="操作" min-width="150">
          <template slot-scope="scope">
            ...
            <el-button slot="reference" size="mini" type="danger" @click="deleteCheck(scope.$index, productType)">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogFormVisible" center
               class="chang-product" title="编辑农产品类别信息">
      <el-form ref='changeFrom' :model="alterForm" :rules="alterRules" class="info-form" inline>
        <el-form-item label="农产品名称：" prop="typeName" style="width: 100%">
          <el-input v-model="alterForm.typeName" placeholder="农产品名称" size="small"></el-input>
        </el-form-item>
        <el-form-item label="农产品描述：" prop="typeDescribe" style="width: 100%">
          <el-input v-model="alterForm.typeDescribe" autosize placeholder="农产品描述" size="small"
        ...
        <el-form-item label="横幅宣传图" prop="typeUrlTop" style="width: 45%;position:absolute;top: 0;right: 20px;">
          <el-upload
            ...
            <el-image v-if="alterForm.typeUrlTop" :src="alterForm.typeUrlTop"
                      style="height: 100px;width: 217px"></el-image>
            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          </el-upload>
        </el-form-item>
        <el-form-item label="左侧宣传图" prop="typeUrlLeft"
                      style="width: 45%;position:absolute;top: 170px;right: 20px;">
          <el-upload
            ref="leftUpload"
            :action="this.$VAR.http+'/uploadImage?name=productTypeImage'"
            :before-upload="beforeAvatarUpload"
            ...
            list-type="picture"
            style="display: flex;justify-content: center;align-items: center;height: 100px;width: 217px">
            <el-image v-if="alterForm.typeUrlLeft" :src="alterForm.typeUrlLeft"
                      style="height: 100px;width: 217px"></el-image>
            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          </el-upload>
        </el-form-item>
    ...
    deleteCheck(index, rows) {
      this.$http.post('/product/existsType?productType=' + this.productType[index].typeName).then((rep) => {
        if (rep.data.code === 200) {
          ...
          if (rep.data.data) {
            this.$alert('类别(' + name + ')\t存在农产品，无法删除', '警告', {
        ...
        if (valid) {
          if (this.insert) {
            this.$http.post('/productType/existsTypeName?typeId=0&typeName=' + this.alterForm.typeName).then((rep) => {
              if (rep.data.code === 200) {
                if (rep.data.data) {
                  this.$alert('类别(' + this.alterForm.typeName + ')\t已存在，不能重复添加', '警告', {
                    confirmButtonText: '确定',
                    type: "warning"
                  });
                } else {
                  ...
                  this.$http.post('/productType/add', this.$qs.stringify(this.alterForm)).then((rep) => {
                    ...
                    if (rep.data.code === 200) {
                      ...
                      this.$notify({
                        title: '成功',
                        message: this.alterForm.typeName + '\t类别添加成功',
                        type: 'success'
                      });
                    } else {
                      this.$notify({title: '错误', message: rep.data.message, type: 'error'});
