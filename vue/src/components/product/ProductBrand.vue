  ...
  <div class="product-brand">
    <el-card class="product-hd" shadow="never">
      <div slot="header" style="text-align: left;">
        ...
        <span>筛选搜索</span>
        <el-button class="el-button-search" style="right: 111px" type="primary" @click="queryProductBrand()">查询
        </el-button>
        <el-button class="el-button-search" style="right: 20px" @click="resetForm('productBrandForm')">重置</el-button>
      </div>
      <el-form ref="productBrandForm" :model="productBrandForm" class="query-form" inline>
        <el-form-item label="输入搜索：" prop="key">
          <el-input v-model="productBrandForm.key" clearable placeholder="品牌名称/关键字"
                    @keyup.enter.native="queryProductBrand"></el-input>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="product-box" shadow="never">
      ...
      <el-table :data="productBrand" border class="brand-table" height="425" style="width: 100%">
        <el-table-column label="编号" prop="brandId" width="150"></el-table-column>
        <el-table-column label="品牌名称" prop="brandName" width="350"></el-table-column>
        <el-table-column label="品牌描述" prop="brandDescribe" width="600"></el-table-column>
        <el-table-column align="center" label="操作" min-width="200">
          <template slot-scope="scope">
            ...
            <el-button slot="reference" size="mini" style="margin: 0 2%" type="danger"
                       @click="deleteBrand(scope.$index, productBrand)">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogFormVisible" center
               class="chang-product" title="编辑农产品品牌信息">
      <el-form ref='changeFrom' :model="alterForm" :rules="alterRules" class="info-form" inline>
        <el-form-item label="品牌名称：" prop="brandName" style="width: 100%">
          <el-input v-model="alterForm.brandName" placeholder="品牌名称" size="small"></el-input>
        </el-form-item>
        <el-form-item label="品牌描述：" prop="brandDescribe" style="width: 100%">
          <el-input v-model="alterForm.brandDescribe" autosize placeholder="品牌描述" size="small"
    ...
    deleteBrand(index, rows) {
      ...
      this.$http.post('/product/existsBrand?productBrand=' + name).then((rep) => {
        if (rep.data.code === 200) {
          if (rep.data.data) {
            this.$alert('品牌(' + name + ')\t下存在农产品，无法删除', '警告', {
        ...
        if (valid) {
          if (this.insert) {
            this.$http.post('/productBrand/existsBrandName?brandId=0&brandName=' + this.alterForm.brandName).then((rep) => {
              if (rep.data.code === 200) {
                if (rep.data.data) {
                  this.$alert('品牌(' + this.alterForm.brandName + ')\t已存在，不能重复添加', '警告', {
                    confirmButtonText: '确定',
                    type: "warning"
                  });
                } else {
                  ...
                  this.$http.post('/productBrand/add', this.$qs.stringify(this.alterForm)).then((rep) => {
                    ...
                    if (rep.data.code === 200) {
                      ...
                      this.$notify({
                        title: '成功',
                        message: this.alterForm.brandName + '\t品牌添加成功',
                        type: 'success'
                      });
                    } else {
                      this.$notify({title: '错误', message: rep.data.message, type: 'error'});
