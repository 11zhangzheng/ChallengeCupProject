<template>
  <div class="product-brand">
    ...
    <el-dialog title="编辑农产品品牌信息" class="chang-product" :close-on-click-modal="false" :visible.sync="dialogFormVisible" center>
      ...
      deleteBrand(index, rows){
        ...
        this.$http.post('/product/existsBrand?productBrand='+name).then((rep)=>{
          if(rep.data.code===200){
            if(rep.data.data){
              this.$alert('品牌('+name+')\t下存在农产品，无法删除', '警告', {confirmButtonText: '确定', type:"warning"});
            }else{
