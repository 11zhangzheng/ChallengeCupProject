...
<template>
  <div class="product-list">
    <el-card class="box-card product-list-card" shadow="never">
      ...
      <el-form ref="ruleForm" :model="ruleForm" class="el-form" inline>
        <el-form-item label="农产品名称：" prop="name">
          <el-input v-model="ruleForm.name" placeholder="农产品名称"></el-input>
        </el-form-item>
        <el-form-item label="农产品货号：" prop="goodsNo">
          <el-input v-model="ruleForm.goodsNo" placeholder="农产品货号"></el-input>
        </el-form-item>
        <el-form-item label="农产品分类：" prop="sort">
          <el-select v-model="ruleForm.sort" clearable filterable placeholder="请选择分类">
            <el-option v-for="item in productSort" :key="item" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="农产品品牌：" prop="brand">
          <el-select v-model="ruleForm.brand" clearable filterable placeholder="请选择品牌">
            <el-option v-for="item in productBrand" :key="item" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="上架状态：" prop="put_status">
          <el-select v-model="ruleForm.put_status" clearable placeholder="全部">
            <el-option v-for="(item,index) in putStatus" :key="index" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="新品状态：" prop="new_status">
          <el-select v-model="ruleForm.new_status" clearable placeholder="全部">
        ...
        <el-form-item label="缺货状态：" prop="stockout_status">
          <el-select v-model="ruleForm.stockout_status" clearable placeholder="全部">
    ...
    <el-card class="box-card product-list-card" shadow="never">
      ...
      <div>
        <el-table slot="empty" :data="tableData" border height="500px" highlight-current-row stripe style="width: 100%">
          <el-table-column align="center" type="selection" width="50"></el-table-column>
          <el-table-column align="center" label="编号" prop="productId" width="50"></el-table-column>
          <el-table-column align="center" label="农产品图片" width="150">
          ...
          <el-table-column align="center" label="农产品名称" width="300">
            <template slot-scope="scope">
              <p>货号：{{ tableData[scope.$index].productNo }}</p>
              <p v-text="tableData[scope.$index].productName"></p>
            </template>
          </el-table-column>
          <el-table-column align="center" label="分类/品牌" width="140">
            <template slot-scope="scope">
          ...
          <el-table-column align="center" label="标签" width="140">
            <template slot-scope="scope">
              <p>上架：
                <el-switch v-model="tableData[scope.$index].isSale" @change="isSaleState(scope.$index)"/>
              </p>
              <p>新品：
                <el-switch v-model="tableData[scope.$index].isNew" @change="isNewState(scope.$index)"/>
              </p>
            </template>
          </el-table-column>
          <el-table-column align="center" label="库存" width="90">
            <template slot-scope="scope">
              <el-tooltip v-if="tableData" :content="'最低库存：'+tableData[scope.$index].lowestStock" class="item" effect="dark"
                          placement="right">
                <p style="cursor: pointer;" v-text="tableData[scope.$index].productStock"></p>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column align="center" label="缺货状态" width="90">
            <template slot-scope="scope">
              <span v-if="tableData[scope.$index].isStockOut" style="color: #ff0000">缺货</span>
              <span v-else>正常</span>
            </template>
          </el-table-column>
          <el-table-column align="center" label="开售时间" prop="saleTime" width="100">
          </el-table-column>
          <el-table-column align="center" label="操作" min-width="150">
            <template slot-scope="scope">
              <div>
                <el-button slot="reference" size="mini" style="margin-bottom: 20px!important;" type="danger"
                           @click="purchase(scope.$index)">农产品采购
                </el-button>
              </div>
              <div>
                <el-button size="mini" @click="openChange(scope.$index)">编辑</el-button>
                <el-button slot="reference" size="mini" type="danger" @click="deleteCheck(scope.$index, tableData)">
                  删除
                </el-button>
              </div>
            </template>
    ...
    <el-dialog v-if="tableData[handleIndex]" :visible.sync="dialogFormVisible" center class="chang-product"
               title="编辑农产品">
      <el-form ref='changeFrom' :model="alterForm" :rules="alterRules" class="el-form" inline>
        <el-form-item label="农产品名称：" prop="productName">
          <el-input v-model="alterForm.productName" placeholder="农产品名称"></el-input>
        </el-form-item>
        <el-form-item label="农产品分类：" prop="productType">
        ...
        <el-form-item label="农产品进价：" prop="inPrice">
          <el-input v-model="alterForm.inPrice" min=0 placeholder="农产品进价" type='number'></el-input>
        </el-form-item>
        <el-form-item label="农产品品牌：" prop="productBrand">
        ...
        <el-form-item label="农产品售价：" prop="outPrice">
          <el-input v-model="alterForm.outPrice" min=0 placeholder="农产品售价" type='number'></el-input>
        </el-form-item>
        <el-form-item label="最低库存：" prop="lowestStock">
          <el-input-number v-model="alterForm.lowestStock" max=1000 min=0 placeholder="最低库存"
                           size="medium"></el-input-number>
        </el-form-item>
        <el-form-item label="农产品描述：" prop="productDescribe">
          <el-input v-model="alterForm.productDescribe" autosize placeholder="农产品描述" type="textarea"></el-input>
        </el-form-item>
        <el-form-item label="农产品图片：" prop="productUrl">
          <el-upload
            ref="productUpload"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button style="padding: 12px 54px;margin: 0 55px;" type="primary" @click="submitChange('changeFrom')">确 定
        </el-button>
        <el-button style="padding: 12px 54px;margin: 0 55px;" @click="cancelChange('changeFrom')">取 消</el-button>
      </div>
    </el-dialog>
    <el-dialog :close-on-click-modal="false" :visible.sync="purchaseFormVisible" center
               class="chang-purchase" title="农产品采购" width="500px">
      <el-form ref='changeFrom' :model="purchaseForm" :rules="purchaseRules" class="info-form" inline>
        <el-form-item label="农产品编号：" prop="productNo">
          <el-input v-model="purchaseForm.productNo" placeholder="农产品编号" readonly size="small"></el-input>
        </el-form-item>
        <el-form-item label="农产品名称：" prop="productName">
          <el-input v-model="purchaseForm.productName" placeholder="农产品名称" readonly size="small"></el-input>
        </el-form-item>
        <el-form-item label="供应商：" prop="supplierNo">
          <el-select v-model="supplierIndex" filterable placeholder="请选择供应商" size="small">
            <el-option v-for="(item,index) in supplierInfo" :key="index" :label="item.supplierName"
                       :value="index"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="采购数量：" prop="purchaseNumber">
          <el-input-number v-model="purchaseForm.purchaseNumber" :max=1000 :min=1 label="采购数量"
                           size="small" style="width:186px" @change="numberChange"></el-input-number>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button style="padding: 10px 35px; margin:0 30px 0 60px;" type="primary" @click="submitPurchaseInfo">确 定
        </el-button>
...
export default {
  ...
  data() {
    return {
      ...
      purchaseFormVisible: false,  //农产品采购界面
      handleIndex: 0,
      ...
      alterRules: {
        ...
        outPrice: [{required: true, message: '请输入农产品售价', trigger: 'blur'}],
        inPrice: [{required: true, message: '请输入农产品进价', trigger: 'blur'}],
        productDescribe: [{required: true, message: '请输入农产品描述', trigger: 'blur'},
  ...
  methods: {
        ...
        if (valid) {
          ...
          this.$http.post('/product/update', this.$qs.stringify(this.alterForm)).then((rep) => {
            ...
            if (rep.data.code === 200) {
              ...
              this.$notify({title: '成功', message: '农产品信息更新成功', type: 'success'});
            } else {
              this.$msg.error(rep.data.message)
            }
          }).catch((err) => {
    ...
    deleteCheck(index, rows) {
      this.$confirm('此操作将永久删除该农产品, 是否继续?', '提示', {
        ...
        this.$http.post('/product/delete?productId=' + this.tableData[index].productId).then((rep) => {
          if (rep.data.code === 200) {
            ...
            this.$notify({title: '成功', message: name + '\t农产品移除成功', type: 'success'});
          }
        }).catch((err) => {
          this.$msg.error(err);
        })
