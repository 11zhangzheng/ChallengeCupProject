  ...
  <div class="product-specs">
    <el-card class="product-hd" shadow="never">
      ...
      <el-form :model="productSpecsForm" ref="productSpecsForm" inline class="query-form">
        <el-form-item label="输入搜索：" prop="key">
          <el-input v-model="productSpecsForm.key" @keyup.enter.native="queryProductSpecs" clearable placeholder="农产品类别/规格类型"></el-input>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="product-specs-box" shadow="never">
      ...
      <el-table :data="productSpecs" class="type-table" height="425"  border style="width: 100%">
        ...
        <el-table-column prop="productType" label="农产品类型" width="500"></el-table-column>
    ...
    <el-dialog title="编辑农产品规格信息" class="chang-specs" :close-on-click-modal="false" top="30vh" width="500px" :visible.sync="dialogFormVisible" center>
                  ...
                  if(rep.data.data){
                    ...
                    this.$http.post('/specs/add',this.$qs.stringify(this.alterForm, { skipNulls: true })).then((rep)=>{
                      ...
                      if(rep.data.code===200){
                        ...
                        this.reqInfo();
                        this.$notify({title: '成功', message: '农产品规格('+this.alterForm.specsName+')\t添加成功', type: 'success'});
                      }else{
                        this.$notify({title: '错误', message: '农产品规格('+this.alterForm.specsName+')\t添加失败', type: 'error'});
                      }
