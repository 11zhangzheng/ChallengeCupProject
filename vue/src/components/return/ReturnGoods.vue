    ...
    <el-card class="box-card return-goods-card" shadow="never">
      ...
      <el-form ref="queryForm" :model="queryForm" class="el-form" inline>
        ...
        <el-form-item label="申请人：" prop="user">
          <el-input v-model="queryForm.user" auto-complete="no" placeholder="申请人帐号/姓名"></el-input>
        </el-form-item>
        <el-form-item label="操作员：" prop="operator">
          <el-input v-model="queryForm.operator" auto-complete="no" placeholder="操作员帐号/姓名"></el-input>
        </el-form-item>
        <el-form-item label="退货状态：" prop="returnState">
          <el-select v-model="queryForm.returnState" clearable filterable placeholder="全部">
            <el-option v-for="item in returnStates" :key="item" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="退货原因：" prop="returnReason">
          <el-select v-model="queryForm.returnReason" clearable placeholder="全部">
    ...
    <el-card class="box-card return-goods-card" shadow="never">
      ...
      <div>
        <el-table slot="empty" :data="tableData" border height="500px" highlight-current-row stripe style="width: 100%">
          <el-table-column align="center" label="编号" prop="returnId" width="50"></el-table-column>
          <el-table-column align="center" label="申请时间" prop="applyTime" width="180"></el-table-column>
          <el-table-column align="center" label="用户帐号" width="180">
            <template slot-scope="scope">
              <el-tooltip v-if="tableData" :content="'姓名：'+tableData[scope.$index].userName" class="item" effect="dark"
                          placement="right">
                <p style="cursor: pointer" v-text="tableData[scope.$index].userNumber"></p>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column align="center" label="订单编号" prop="orderNo" width="140"></el-table-column>
          <el-table-column align="center" label="退款金额" prop="returnPrice" width="100"></el-table-column>
          <el-table-column align="center" label="退货原因" prop="returnReason" width="140"></el-table-column>
          <el-table-column align="center" label="操作员" width="180">
            <template slot-scope="scope">
              <el-tooltip v-if="tableData" :content="'姓名：'+tableData[scope.$index].operatorName" class="item" effect="dark"
                          placement="right">
                <p style="cursor: pointer" v-text="tableData[scope.$index].operatorNumber"></p>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column align="center" label="申请状态" width="90">
            <template slot-scope="scope">
          ...
          <el-table-column align="center" label="处理时间" prop="dealTime" width="180"></el-table-column>

          <el-table-column align="center" label="操作" min-width="150">
            <template slot-scope="scope">
              <el-button v-if="scope.row.returnState==='待收货'" size="mini" type="primary"
                         @click="receiveGoods(scope.$index)">确认收货
              </el-button>
              <el-button v-if="scope.row.returnState==='待处理'" size="mini" @click="agree(scope.$index)">同意
              </el-button>
              <el-button v-if="scope.row.returnState==='待处理'" size="mini" type="danger"
                         @click="reject(scope.$index)">拒绝
              </el-button>
              <el-button v-if="scope.row.returnState==='待退款'" size="mini" @click="refund(scope.$index)">退款
              </el-button>
              <el-button v-if="scope.row.returnState==='待退款'" size="mini" type="danger"
                         @click="rejectRefund(scope.$index)">拒绝
              </el-button>
              <el-button v-else-if="scope.row.returnState!=='待收货' && scope.row.returnState!=='待处理'" size="mini"
                         @click="orderInfo(scope.$index)">订单详情
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      this.$confirm('同意买家的退货申请？', '提示', {
        ...
        this.$http.post('/returnGoods/dealWithReturn?returnId=' + this.tableData[index].returnId + '&operatorNumber=' + operatorNumber + '&operatorName=' + operatorName).then((rep) => {
          ...
          if (rep.data.code === 200) {
            ...
            this.$notify({
              title: '成功',
              message: this.tableData[index].userName + '\t退货申请已处理',
              type: 'success'
            });
          }
        }).catch((err) => {
        ...
        this.$http.get('/alipay/refund?orderNo=' + orderNo + "&payPrice=" + returnPrice).then((rep) => {
          ...
          if (rep.data.code === 200) {
            ...
            this.$http.post('/returnGoods/dealRefund?returnId=' + this.tableData[index].returnId + '&operatorNumber=' + operatorNumber + '&operatorName=' + operatorName).then((rep) => {
              ...
              if (rep.data.code === 200) {
                this.reqInfo()
                this.$notify({title: '成功', message: '农产品退款成功，退款金额：￥' + returnPrice, type: 'success'});
              }
            }).catch((err) => {
              loading.close();
              this.$msg.error(err)
      ...
      this.$confirm('拒绝买家的退货申请？', '提示', {
        ...
        this.$http.post('/returnGoods/refuseReturn?returnId=' + this.tableData[index].returnId + '&operatorNumber=' + operatorNumber + '&operatorName=' + operatorName).then((rep) => {
          ...
          if (rep.data.code === 200) {
            ...
            this.$notify({
              title: '成功',
              message: this.tableData[index].userName + '\t退货申请已拒绝',
              type: 'success'
            });
          }
        }).catch((err) => {
      ...
      this.$confirm('拒绝买家的退款申请？', '提示', {
        ...
        this.$http.post('/returnGoods/rejectRefund?returnId=' + this.tableData[index].returnId + '&operatorNumber=' + operatorNumber + '&operatorName=' + operatorName).then((rep) => {
          ...
          if (rep.data.code === 200) {
            ...
            this.$notify({
              title: '成功',
              message: this.tableData[index].userName + '\t退款申请已拒绝',
              type: 'success'
            });
          }
        }).catch((err) => {
