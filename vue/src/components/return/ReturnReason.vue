  ...
  <div class="return-reason">
    <el-card class="reason-hd" shadow="never">
      <div slot="header" style="text-align: left;">
        ...
        <span>筛选搜索</span>
        <el-button class="el-button-search" style="right: 111px" type="primary" @click="queryReturnReason()">查询
        </el-button>
        <el-button class="el-button-search" style="right: 20px" @click="resetForm('returnReasonForm')">重置</el-button>
      </div>
      <el-form ref="returnReasonForm" :model="returnReasonForm" class="query-form" inline>
        <el-form-item label="输入搜索：" prop="key">
          <el-input v-model="returnReasonForm.key" clearable placeholder="退货原因"
                    @keyup.enter.native="queryReturnReason"></el-input>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="reason-box" shadow="never">
      ...
      <el-table :data="returnReason" border class="type-table" height="425" style="width: 100%">
        <el-table-column label="编号" prop="reasonId" width="200"></el-table-column>
        <el-table-column label="原因类型" prop="reasonName" width="600"></el-table-column>
        <el-table-column label="是否启用" width="350">
          <template slot-scope="scope">
            <el-switch v-model="returnReason[scope.$index].status" @change="isReasonState(scope.$index)"/>
          </template>
        </el-table-column>
        <el-table-column align="center" label="操作" min-width="200">
          <template slot-scope="scope">
            ...
            <el-button slot="reference" size="mini" style="margin: 0 4%" type="danger"
                       @click="deleteReason(scope.$index, returnReason)">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogFormVisible" center class="chang-reason"
               title="编辑退货原因" top="28vh" width="420px">
      <el-form ref='changeFrom' :model="alterForm" :rules="alterRules" class="info-form" inline>
        <el-form-item label="退货原因：" prop="reasonName" style="width: 100%">
          <el-input v-model="alterForm.reasonName" placeholder="退货原因" size="small"></el-input>
        </el-form-item>
      </el-form>
        ...
        if (valid) {
          if (this.insert) {
            this.$http.post('/returnReason/existReasonName?reasonId=0&reasonName=' + this.alterForm.reasonName).then((rep) => {
              if (rep.data.code === 200) {
                if (rep.data.data) {
                  this.$alert('退货原因(' + this.alterForm.reasonName + ')\t已存在，不能重复添加', '警告', {
                    confirmButtonText: '确定',
                    type: "warning"
                  });
                } else {
                  ...
                  this.$http.post('/returnReason/addReason', this.$qs.stringify(this.alterForm)).then((rep) => {
                    ...
                    if (rep.data.code === 200) {
                      ...
                      this.$notify({
                        title: '成功',
                        message: '退货原因(' + this.alterForm.reasonName + ')\t添加成功',
                        type: 'success'
                      });
                    } else {
                      this.$notify({title: '错误', message: rep.data.message, type: 'error'});
                ...
                if (rep.data.data) {
                  ...
                  this.$http.post('/returnReason/updateReason', this.$qs.stringify(this.alterForm)).then((rep) => {
                    ...
                    if (rep.data.code === 200) {
                      ...
                      this.$notify({
                        title: '成功',
                        message: '退货原因(' + this.alterForm.reasonName + ')\t更新成功',
                        type: 'success'
                      });
                    } else {
                      this.$notify({title: '错误', message: rep.data.message, type: 'error'});
